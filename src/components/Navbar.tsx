...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed top-4 left-1/2 -translate-x-1/2 z-50 rounded-full bg-background/10 backdrop-blur-xl py-4 px-6 md:px-10 transition-all duration-300 ease-in-out ${navbarWidthClasses}`}>
      ...
      {isMobile && isOpen && (
        <div className="absolute top-full left-0 w-full bg-background/10 backdrop-blur-xl border-b border-border/40 flex flex-col items-center space-y-4 py-6 animate-accordion-down rounded-b-xl">
          {navLinks}
